...
int solutionAccepte(char *sol, Session *s, Joueur *myJoueur); // A FAIRE
...
// j pour connaitre son enchere
int solutionAccepte(char *sol, Session *s, Joueur *myJoueur) { // Mettre dans fonction_jeu.c
  return 0;
}
...
Joueur *getNewJoueurActif(Session *s) {
  ...
  cur = l->j;
  while(cur != NULL) {
    if((cur->enchere > 0) && (cur->enchere < enchereMin)) {
      res = cur;
